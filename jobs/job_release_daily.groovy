...
ArrayList LIST_PUX_NO_DIAGNOSTIC_UNITTEST = ["${OTF_BRANCH}\\test\\PUX Storage\\PUX_6.99\\OtherUnitTestCases\\OtxRuntimeUnitTestLogic.pux",
											"${OTF_BRANCH}\\test\\PUX Storage\\PUX_6.99\\OtherUnitTestCases\\OtxRuntimeUnitTestQuantities.pux", 
											"${OTF_BRANCH}\\test\\PUX Storage\\PUX_6.99\\OtherUnitTestCases\\OtxRuntimeUnitTestTestResultHandling.pux"]
...
def LOCK_KEY_OTF_SOLUTION = "Lock build solution OTF inside  ${OTF_SOURCE}"
		...
		stage("Update file version") {
			when {
				expression { return CANCEL_BUILD == false }
			}
			steps {
				catchError { build job: SUB_JOBS.CHANGE_VERSION_SOME_FILES, parameters: [
		...
		stage('Send mail') {
			when {
				expression { return params.MAIL_TO.length() > 0 && CANCEL_BUILD == false }
			}
			steps {
				catchError { build job: SUB_JOBS.SEND_EMAIL, parameters: [
